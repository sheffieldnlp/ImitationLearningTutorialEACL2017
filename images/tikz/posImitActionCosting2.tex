...
\matrix(m)[matrix of math nodes,column sep=1cm,row sep=0.3cm]{
    ...
    \textbf{Verb} & \mathord{\cdot}  &\color{red}{\circ} &    \mathord{\bigstar} &  \hspace{-0.4in} cost=1  \\
...
(m-2-3)edge  node [xshift=+0.2in,yshift=-0.12in] {rollout}  (m-2-4)
(m-4-2)edge  node [auto, xshift=-0.1in] {rollin}  (m-2-3);
